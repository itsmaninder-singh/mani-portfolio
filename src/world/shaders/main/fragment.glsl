...
void main() {
    ...
    float sharpParticle = 1.0 - smoothstep(0.12, 0.22, d);

    float bloomHalo = (1.0 - smoothstep(0.04, 0.35, d)) *
        uBloomStrength *
        uTextSharpness *
        0.35;

    float haze = exp(-d * d * 7.5) *
        (1.0 - smoothstep(0.35, 0.45, d)) *
        uHazeStrength *
        uTextSharpness *
        0.3;
    ...
    vec3 textBloom = mix(uBloomColor * 0.7, prism, 0.45);

    vec3 bloomColor = mix(textBloom, textSurface, max(whiteCore, 0.4));

    vec3 color = mix(uColor, bloomColor, uTextSharpness);

    color *= mix(1.0, 0.92, pow(curvedGlassEdge, 1.45));
