...
describe('customer confirmation markup', () => {
    it('allows a single long token to wrap inside the message block', () => {
        const html = renderCustomerConfirmationBodyHtml(
            'https://itsmaninder-singh.github.io/https://itsmaninder-singh.github.io/https://itsmaninder-singh.github.io/',
        );
...
describe('contact Pages Function', () => {
    ...
    it('verifies the visitor IP and sends one email when confirmation is disabled', async () => {
        ...
        assert.equal(ownerEmail.subject, 'Project inquiry via itsmaninder-singh.github.io');
    ...
    it('sends visitor confirmation only when explicitly enabled after Turnstile passes', async () => {
        ...
        assert.match(String(visitorEmail.html), /https:\/\/itsmaninder-singh\.github\.io/);
        assert.match(
            String(visitorEmail.html),
            /https:\/\/itsmaninder-singh\.github\.io\/email\/v\d+-\d{4}-\d{2}-\d{2}\/ms-monogram\.png/,
        );
        assert.match(
            String(visitorEmail.html),
            /https:\/\/itsmaninder-singh\.github\.io\/email\/v\d+-\d{4}-\d{2}-\d{2}\/globe\.png/,
        );
        assert.match(
            String(visitorEmail.html),
            /https:\/\/itsmaninder-singh\.github\.io\/email\/v\d+-\d{4}-\d{2}-\d{2}\/signature-wave\.png/,
        );
